...
interface Statistics {
    ...
    habitStats: Record<string, HabitItemStats>;
    dailyStats: Record<string, DailyStats>;
    streaks: Record<string, number>;
}

interface Cache {
    records: HabitRecord[] | null;
...
class HabitParser {
    ...
    constructor(config: HabitConfig) {
        this.config = config;
    }

    parseRecord(line: string, fileDate: string): HabitRecord[] | null {
    ...
    parseFileContent(content: string, filePath: string): HabitRecord[] {
...
class HabitStorage {
    ...
    constructor(app: App, config: HabitConfig) {
        this.app = app;
        this.config = config;
    ...
    async getAllRecords(forceRefresh: boolean = false): Promise<HabitRecord[]> {
        if (forceRefresh) {
            this.clearCache();
        }
    ...
    filterRecordsByDateRange(records: HabitRecord[], startDate: string, endDate: string): HabitRecord[] {
    ...
    calculateStatistics(records: HabitRecord[]): Statistics {
        const stats: Statistics = {
            totalCheckins: records.length,
            habitStats: {} as Record<string, HabitItemStats>,
        ...
        records.forEach(record => {
            ...
            const hs = stats.habitStats[record.habitKey];
            if (hs) {
                hs.count += 1;
                if (!hs.dates.includes(record.date)) {
                    hs.dates.push(record.date);
                }
            }
            ...
            const ds = stats.dailyStats[record.date];
            if (ds && !ds.habits.includes(record.habitKey)) {
                ds.habits.push(record.habitKey);
                ds.count += 1;
            }
        });

        Object.keys(stats.habitStats).forEach(habitKey => {
            const hs = stats.habitStats[habitKey];
            if (hs) {
                const sorted = hs.dates.sort();
                stats.streaks[habitKey] = this.calculateStreak(sorted);
            }
        });
    ...
    calculateStreak(dates: string[]): number {
        ...
        for (let i = 1; i < sortedDates.length; i++) {
            ...
            const prevDate = new Date(sortedDates[Number(i) - 1]);
...
class HabitConfigModal extends Modal {
    ...
    deleteHabit(key: string): void {
        delete this.habits[key];
        this.renderHabitList();
    }

    updateHabit(oldKey: string, newKey: string, name: string): void {
        if (oldKey !== newKey) {
    ...
    async saveConfig(): Promise<void> {
        try {
            ...
            const cleanHabits: Record<string, string> = {};
...
class HabitTrackerView extends ItemView {
    ...
    currentMonth: Date;
    filteredRecords!: HabitRecord[];
    currentTimeRange!: TimeRange;
    statsContainer!: HTMLElement;
    habitListContainer!: HTMLElement;
    checkInRecordsContainer!: HTMLElement;
    ...
    renderTimeFilter(container: HTMLElement): void {
    ...
    renderHabitList(container: HTMLElement): void {
        this.habitListContainer = container.createDiv('habit-list-view');
        this.updateHabitListDisplay();
    }

    renderCheckInRecords(container: HTMLElement): void {
    ...
    async loadAllRecords(forceRefresh: boolean = false): Promise<void> {
        try {
            if (forceRefresh) {
                new Notice('正在刷新打卡数据...');
            }
    ...
    updateHabitListDisplay(): void {
        ...
        const habitStats = this.currentStats?.habitStats || {} as Record<string, HabitItemStats>;
        const streaks = this.currentStats?.streaks || {} as Record<string, number>;
        const dailyStats = this.currentStats?.dailyStats || {} as Record<string, DailyStats>;
        ...
        Object.entries(habits)
            ...
            .forEach(([habitKey, habitName]) => {
                ...
                last7Days.forEach(dateStr => {
                    ...
                    checkbox.onchange = () => {
                        void this.toggleHabitCheck(habitKey, dateStr, checkbox);
                    };
                });
            });
    }

    async toggleHabitCheck(habitKey: string, dateStr: string, checkboxElement: HTMLInputElement): Promise<void> {
        try {
            ...
            if (!file) {
                ...
                const dateTitle = `${year}年${parseInt(month, 10)}月${parseInt(day, 10)}日`;
    ...
    updateCheckInRecordsDisplay(): void {
        ...
        const recordsByDate: Record<string, HabitRecord[]> = {};
...
class HabitTrackerPlugin extends Plugin {
    ...
    async onload(): Promise<void> {
        ...
        this.registerView(HABIT_VIEW, (leaf: WorkspaceLeaf) => new HabitTrackerView(leaf, this));
    ...
    async loadConfig(): Promise<void> {
        try {
            ...
            if (await adapter.exists(configPath)) {
                ...
                this.config = JSON.parse(configContent) as HabitConfig;
